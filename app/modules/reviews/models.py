...
class ReviewAssignment(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "application_id", "reviewer_id", name="uq_review_assignments_application_reviewer"
        ),
    ...
    template_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("scorecard_templates.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
...
class ReviewResponse(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "assignment_id", "criterion_id", name="uq_review_responses_assignment_criterion"
        ),
    )
    ...
    criterion_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("scorecard_criteria.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
