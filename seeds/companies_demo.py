...
HOW_WE_WORK_REMOTE_FIRST = [
    {
        "icon": "globe",
        "title": "Remote-first",
        "description": "Pracujemy z dowolnego miejsca na świecie.",
    },
    {
        "icon": "calendar-off",
        "title": "No-meetings Wednesdays",
        "description": "Środa bez spotkań to świętość.",
    },
    {
        "icon": "messages",
        "title": "Async-first",
        "description": "Komunikacja asynchroniczna to nasz standard.",
    },
...
HOW_WE_WORK_HYBRID = [
    {"icon": "building", "title": "Hybryda 2+3", "description": "Dwa dni w biurze, trzy zdalnie."},
    {
        "icon": "users",
        "title": "Małe zespoły",
        "description": "Maks. 8 osób na squad — nikt się nie gubi.",
    },
    {
        "icon": "graduation-cap",
        "title": "Czas na naukę",
        "description": "10% czasu pracy na rozwój własny.",
    },
    {
        "icon": "heart",
        "title": "Psychological safety",
        "description": "Pytania głupie nie istnieją.",
    },
]
...
FAQ_STANDARD = [
    {
        "question": "Czy oferujecie pracę w pełni zdalną?",
        "answer": "Tak — wszystkie role inżynierskie i produktowe są w pełni remote-friendly. Współpracujemy z osobami z całej UE.",
    },
    {
        "question": "Jak wygląda onboarding?",
        "answer": "Pierwszy tydzień to integracja z zespołem i poznanie produktu. Każdy nowy pracownik dostaje buddy'ego, który prowadzi go przez pierwszy miesiąc.",
    },
    {
        "question": "Jakie są możliwości rozwoju?",
        "answer": "Coroczne IDP, budżet szkoleniowy, ścieżki kariery zarówno IC, jak i management. Konferencje opłacane przez firmę.",
    },
    {
        "question": "Jak wygląda typowy dzień pracy?",
        "answer": "Krótkie standup'y rano (15 min), bloki focus time po południu. Środy bez spotkań. Komunikacja głównie async.",
    },
]
...
def build_demo_companies() -> list[dict[str, Any]]:
    return [
        ...
        _minimal_profile(
            name="Vue Storefront", industry="E-commerce", hq="Wrocław, Polska", employees=70
        ),
    ]
...
async def main(reset: bool = False) -> None:
    ...
    async with Session() as db:
        if reset:
            ...
            await db.execute(
                text(
                    f"DELETE FROM users WHERE company_id IN (SELECT id FROM companies WHERE name LIKE '%{SEED_TAG}%')"
                )
            )
        ...
        for profile in build_demo_companies():
            ...
            existing = (
                await db.execute(select(Company).where(Company.slug == profile["slug"]))
            ).scalar_one_or_none()
            if existing:
