...
@pytest.mark.asyncio
async def test_recruiter_can_assign_manager_review_and_manager_can_submit(
    client, db_session
) -> None:
    company = await create_company(db_session, "Reviews Co")
    recruiter = await create_verified_user(
        db_session, company.id, "recruiter@reviews.com", "recruiter"
    )
...
async def test_only_assigned_manager_can_submit_scorecard(client, db_session) -> None:
    ...
    recruiter = await create_verified_user(
        db_session, company.id, "recruiter@guard.com", "recruiter"
    )
    assigned_manager = await create_verified_user(
        db_session, company.id, "assigned@guard.com", "manager"
    )
    ...
    submit_response = await client.post(
        ...
        json={
            "responses": [
                {"criterion_id": criterion["id"], "score": 3} for criterion in template["criteria"]
            ],
            "recommendation": "no",
        },
    )
